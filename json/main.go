...
func main() {
	...
	res2B, _ := json.Marshal(res2D) //传结构体本身或者指针都行，但是良好的代码风格是传指针
	...
	// 访问嵌套的值需要一系列的转化
	strs := dat["strs"].([]interface{}) //interface类型
	fmt.Println(dat["num"])             //interface类型
	fmt.Println(dat["strs"])            //interface类型
	fmt.Println(strs)                   //interface类型
	str1 := strs[0].(string)            //string类型
	fmt.Println(strs[0])                //interface类型
	fmt.Println(str1)                   //string类型
	...
	json.Unmarshal([]byte(str), &res2) //参数必须为指针类型
	fmt.Println(res)
	fmt.Println(res2)
